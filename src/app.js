(function(){
  ...
  App.prototype.bind=function(){
    ...
    this.ui.on("select",function(o){
      ...
      if(o.key==="wallMode"&&o.value==="SAME"){
        self.state.wallBack=self.state.wallLeft;
        self.state.wallRight=self.state.wallLeft;
      }

      if(o.key==="wallLeft"&&self.state.wallMode==="SAME"){
    ...
    this.ui.on("reset",function(){
      ...
      self.state=JSON.parse(JSON.stringify(CONFIG.DEFAULT_STATE));
      localStorage.removeItem("elevator_config_state");
  ...
  App.prototype.total=function(){
    ...
    var price=function(g,id){
      var x=(c[g]||[]).find(function(item){return item.id===id;});
      return x?x.price:0;
    };
    ...
    t+=s.wallMode==="SAME"
      ?price("WALLS",s.wallLeft)*3
      :price("WALLS",s.wallLeft)+price("WALLS",s.wallBack)+price("WALLS",s.wallRight);
  ...
  App.prototype.rebuild=function(){
    ...
    this.cabin.buildCabin(this.state,generationToken)
      ...
      .catch(function(e){
        ...
        console.error("Elevator Configurator build error:",e);
  ...
  App.prototype.openQuote=function(){
    ...
    var find=function(g,id){
      return(c[g]||[]).find(function(x){return x.id===id;});
    };
    ...
    var w=find("WALLS",s.wallLeft);

    if(w&&w.price>0){
      rows.push({
        label:s.wallMode==="SAME"
          ?"3 vách — "+w.name
          :"Vách trái — "+w.name,
        price:w.price*(s.wallMode==="SAME"?3:1)
      });
    }

    if(s.wallMode==="INDEPENDENT"){
      var wb=find("WALLS",s.wallBack);
      var wr=find("WALLS",s.wallRight);

      if(wb&&wb.price>0){
        rows.push({
          label:"Vách sau — "+wb.name,
          price:wb.price
        });
      }
